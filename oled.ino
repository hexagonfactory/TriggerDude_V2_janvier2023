...
void drawPage_mainScreen() {
  if (update_screen_INIT) {
    ...
    /*u8g2.drawRFrame(99, 17, 25, 25, 2); // TRACK FRAME
      ...
      u8g2.print("TRK");*/
    drawBloc_selected_Track(99, 17, 1, 1);
  }
  ...
  if (update_screen_PADS) {
    ...
    uint8_t selTrack = selected_Track + 1;
    ...
    if (SHIFT_pushed) {
      update_screen_ENC_ALT_BTN = true; // on relance if (update_screen_SHIFT_BTN)
    }
  ...
  if (update_screen_ENC_ALT_BTN) {
    ...
    update_screen_ENC_ALT_BTN = false;
    if (!update_screen_INIT)
  ...
  if (update_screen_POTS_CLK_R && screenConfig == 1 && !SHIFT_pushed && !(CVin[0].enable && CVin[0].dest_Selected == 2) && !(CVin[1].enable && CVin[1].dest_Selected == 2)) {
    if (!update_screen_INIT)
      u8g2.clearBuffer();
    (pattern_Length_Locked) ? display_enc2_click(89, "UNLOCK") : display_enc2_click(99, "LOCK");
...
void drawPage_GLOBAL_Settings() {
  ...
  if (update_screen_POTS_DIAL_L || update_screen_POTS_DIAL_R || update_screen_POTS_CLK_R) {
    ...
    u8g2.drawHLine(0, 9, 128);
    draw_GLOBAL_Settings_list();
    print_GLOBAL_Setting_scrollBar();
...
void display_BPM(uint8_t pix_x, uint8_t pix_y) {
...
void display_track_or_pitch() {
  uint8_t dataToDisplay = selected_Track + 1;
...
void drawPage_stepSequencer() { // ---- STEP SEQUENCER
  ...
  if (update_screen_POTS_DIAL_L || update_screen_POTS_CLK_R) {
    ...
    drawBloc_selected_Track(37, 16, 0, 1); // plutôt mettre cadre dans update_screen_INIT et updater le track num
...
void drawPage_pitchSequencer() { // ---- PITCH SEQUENCER
  if (update_screen_INIT) {
    ...
    drawBloc_selected_Track(37, 16, 0, 1);
    drawBloc_CvTracks(59, 12);
...
void drawPage_loopLength() {
  ...
  if (update_screen_POTS_DIAL_L) {
    ...
    u8g2.setFont(u8g2_font_pressstart2p_8u);
    uint8_t trackLgth = bank[currentBank].pattern[currentPattern].track_Length[selected_Track];
    if (trackLgth < 10)
  ...
  if (update_screen_POTS_DIAL_R) {
    ...
    if (bank[currentBank].pattern[currentPattern].tracksRetrig) {
      uint8_t patLgth = bank[currentBank].pattern[currentPattern].pattern_Length;
      if (patLgth < 10)
...
void drawPage_fillState() {
  ...
  if (update_screen_PADS) {
    ...
    if (bank[currentBank].pattern[currentPattern].fill_active)
      display_enc2_click(95, "RESET");

    if (!update_screen_INIT)
      u8g2.updateDisplayArea(13, 4, 2, 4);
...
void drawPage_chromaMode_KBoard() {
  if (update_screen_INIT) {
    ...
    drawBloc_selected_Track(48, 14, 1, 1);
...
void drawPage_LFO_Settings(bool lfoNum) {
  if (update_screen_INIT) {
    if (!lfoNum)
      set_default_UI_header("LFO 1");
    else
      set_default_UI_header("LFO 2");
  ...
  if (update_screen_POTS_DIAL_L || update_screen_POTS_DIAL_R || update_screen_POTS_CLK_R) {
    ...
    u8g2.drawHLine(0, 9, 128);
    draw_LFO_Settings_list(lfoNum);
    print_LFO_Setting_scrollBar();
    u8g2.drawHLine(0, 46, 128);
...
void draw_pads_UI(bool showOnlyselected_Track) {
  for (uint8_t y = 16; y < 36; y += 9) {
    for (uint8_t x = 0; x < 28; x += 9) {
  ...
  if (showOnlyselected_Track) {
    uint8_t AB_State = step_seq_ab[selected_Track];
    ...
    for (uint8_t track = 0; track < TRACKS; ++track) {
      uint8_t AB_State = /*step_seq_ab[track]*/bank[currentBank].pattern[currentPattern].AB_State[track];
      uint8_t e1, e2, e3;
...
void drawBloc_selected_Track(uint8_t px_x, uint8_t px_y, bool listening, bool showTrackNum) {
  u8g2.drawRFrame(px_x, px_y, 25, 25, 2);

  u8g2.setFont(triggerDude_small_font);
  u8g2.setCursor(px_x + 5, px_y + 10);
  u8g2.print("TRK");

  if (showTrackNum) {
    uint8_t x = (listening) ? bank[currentBank].pattern[currentPattern].AB_State[selected_Track] : step_seq_ab[selected_Track];
    (selected_Track + 1 < 10) ?
...
void draw_CLEAR_DELETE_UI() {
  if (update_screen_POTS_DIAL_L || update_screen_POTS_DIAL_R) {
    ...
    u8g2.setFont(triggerDude_small_font);
    for (uint8_t y = 20, z = 0; y < 50; y += 10) {
...
void display_songMode_Grid() {
  ...
  for (uint8_t y = 14; y < 32; y += 16) {
    for (uint8_t x = 2; x < 121; x += 16) {
      ...
      uint8_t stepPattern = bank[currentBank].songPatterns[boxId];
...
void draw_3D_box(uint8_t pix_x, uint8_t pix_y, uint8_t box_w, uint8_t box_h, const char* text, uint8_t data_box_w) {
  // title box 3D
  ...
  // data box
  uint8_t x_offset = pix_x + box_w + 2;
...
void drawBloc_CvTracks(uint8_t pix_x, uint8_t pix_y) {
  draw_3D_box(pix_x, pix_y, 15, 14, "CV", 52);

  uint8_t cvOutCh_1 = bank[currentBank].pattern[currentPattern].cvOut_Tracks[0];
  if (cvOutCh_1 == 12) {
    u8g2.print("-");
  }
  else {
  ...
  uint8_t cvOutCh_2 = bank[currentBank].pattern[currentPattern].cvOut_Tracks[1];
  if (cvOutCh_2 == 12) {
  ...
  uint8_t cvOutCh_3 = bank[currentBank].pattern[currentPattern].cvOut_Tracks[2];
  if (cvOutCh_3 == 12) {
  ...
  uint8_t cvOutCh_4 = bank[currentBank].pattern[currentPattern].cvOut_Tracks[3];
...
void drawBloc_SeqPage(uint8_t pix_x, uint8_t pix_y) {
  draw_3D_box(pix_x, pix_y, 25, 14, "PAGE", 42);

  uint8_t stepSeq_pageDisplay = stepSeq_page + 1;
  makeSpaceBefore(stepSeq_pageDisplay);
  u8g2.print(stepSeq_pageDisplay);
  u8g2.print("-");
  u8g2.print(stepSeq_pageDisplay + 15);
}
...
void makeSpaceBefore(uint8_t elem) {
...
void display_global_Step_4PPQN() {
  u8g2.setFont(triggerDude_big_font);
  u8g2.setCursor(84, 7);
  uint8_t current_Step_Display = (displayFirstStep_1) ? 1 : get_current_Step_Display_4PPQN(1);
...
void display_CurrentBank(uint8_t pix_x, uint8_t pix_y) {     //  0 - 12 - 27 - 14
  draw_3D_box(pix_x, pix_y, 27, 14, "BANK", 13);
  u8g2.print(get_currentBank_Display(1));
}


void display_CurrentPattern(uint8_t pix_x, uint8_t pix_y) {
...
void print_Dial_param_title(uint8_t x, uint8_t y, const char* title) {
  u8g2.setFont(triggerDude_small_font);
  u8g2.setCursor(x, y);
  u8g2.print(title);
}
...
void display_enc2_dial(uint8_t x, const char* title, bool isAbove) {
  uint8_t y = (isAbove) ? 55 : 63;
  u8g2.setFont(u8g2_font_unifont_t_symbols);
...
void display_enc2_click(uint8_t x, const char* title) {
  u8g2.setFont(u8g2_font_unifont_t_symbols);
