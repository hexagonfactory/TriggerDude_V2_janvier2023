...
void set_stepSeq_page() {
  //uint8_t pLength = (bank[currentBank].pattern[currentPattern].tracksRetrig) ? bank[currentBank].pattern[currentPattern].pattern_Length : get_track_Length(selected_Track, 4);
  uint8_t pLength = get_track_Length(selected_Track, 4);
  if (enc2_dir == 1) {
    if (stepSeq_page < (pLength - 16)) {
      stepSeq_page += 16;
    }
  }
...
void set_Step_pitch(uint8_t curTrack, uint8_t curstep4PPQN) {
  //static unsigned long button_micro = 0;
  uint8_t realTrack = curTrack + step_seq_ab[curTrack];
  ...
  uint8_t pot_target_h = (pitch_Range_12) ? (pitchPointer_Pot + 12) : (pitchPointer_Pot);
  uint8_t pot_target_l = (pitch_Range_12) ? (pitchPointer_Pot - 12) : (pitchPointer_Pot);
...
void calc_GLIDE_time() {
  for (uint8_t dac = 0; dac < 4; ++dac) {
    if (DaC[dac].glideActive) {
      uint8_t track = bank[currentBankDisplay].pattern[currentPatternDisplay].cvOut_Tracks[dac]; // 0 (tracks) à 12 (12 = pas de glide sur ce track)
      if (track < TRACKS) {
...
void set_glide_STEPS() {
  uint8_t gSteps = bank[currentBankDisplay].pattern[currentPatternDisplay].glide_steps[selected_Track];
...
void set_ChromaMode_UI() {
  uint8_t arrSize = sizeof(chromaMode_UIs) / sizeof(chromaMode_UIs[0]);
...
void set_rollRate() {
  uint8_t arrSize = sizeof(rollRates) / sizeof(rollRates[0]);
...
void replicate_bars(/*uint16_t*/uint8_t last_pat_length) { // last_pat_length = 16
  //uint8_t bars = last_pat_length / 8;
  //uint8_t bars = (last_pat_length * 4) / 8;
  uint8_t bars = last_pat_length / 2;
...
void set_pattern_Length() {
  uint8_t last_pattern_Length = bank[currentBank].pattern[currentPattern].pattern_Length; // on garde l'actuel pattern_Length avant changement
  ...
  if (!pattern_Length_Locked) {
    if (enc2_dir == 1) {
      ...
      uint8_t limit = (bank[currentBank].pattern[currentPattern].firstStep == 0) ? 1 : (bank[currentBank].pattern[currentPattern].firstStep + 1);
    ...
    if (enc2_dir == 1) {
      ...
      uint8_t new_pattern_Length = last_pattern_Length / 2;
      uint8_t limit = (bank[currentBank].pattern[currentPattern].firstStep == 0) ? 1 : (bank[currentBank].pattern[currentPattern].firstStep + 1);
...
void set_track_Length() {
  uint8_t tLength = bank[currentBank].pattern[currentPattern].track_Length[selected_Track];
...
void set_shift_value(uint8_t track) {
  //uint8_t pLength = (bank[currentBank].pattern[currentPattern].tracksRetrig) ? bank[currentBank].pattern[currentPattern].pattern_Length : get_track_Length(track, 4);
  uint8_t pLength = get_track_Length(track, 4);
...
void set_delete_param() {
  uint8_t arrSize = sizeof(clear_trigsOrPitch_param) / sizeof(clear_trigsOrPitch_param[0]);
...
void set_clear_param() {
  uint8_t arrSize = sizeof(clear_SelectTrack_param) / sizeof(clear_SelectTrack_param[0]);
...
void set_arp_Rate() {
  uint8_t arrSize = sizeof(arp_Rate) / sizeof(arp_Rate[0]);
...
void set_arp_Style() {
  uint8_t arrSize = sizeof(arp_Style) / sizeof(arp_Style[0]);
...
void set_GLOBAL_Settings_selection() { // selection du param GLOBAL à éditer
  uint8_t arrSize = sizeof(GLOBAL_Settings_titles) / sizeof(GLOBAL_Settings_titles[0]);

  if ((enc1_dir == 1) && (GLOBAL_Settings_titles_Cursor < (arrSize - 1))) {
    GLOBAL_Settings_titles_Cursor++;
  }

  else if ((enc1_dir == -1) && (GLOBAL_Settings_titles_Cursor > 0)) {
    GLOBAL_Settings_titles_Cursor--;
  }
...
void set_LFO_Settings_selection() { // selection du param LFO à éditer
  uint8_t arrSize = sizeof(LFO_Settings_titles) / sizeof(LFO_Settings_titles[0]);

  if ((enc1_dir == 1) && (LFO_Settings_titles_Cursor < (arrSize - 1))) {
    LFO_Settings_titles_Cursor++;
  }
...
void set_param_firstStep() {
  ...
  uint8_t firstStep_increment = (enc2_Range_10) ? 10 : 1;
  //uint8_t pLength = (bank[currentBank].pattern[currentPattern].tracksRetrig) ? bank[currentBank].pattern[currentPattern].pattern_Length : selected
...
void set_param_cvin(bool cvInNum) {
  uint8_t arrSize = sizeof(cvIn_modulation_Target) / sizeof(cvIn_modulation_Target[0]);
...
void set_lfo_dest(uint8_t lfoNum) {
  uint8_t arrSize = sizeof(LFOs_output) / sizeof(LFOs_output[0]);
...
void set_lfo_wave(uint8_t lfoNum) {
  uint8_t arrSize = sizeof(LFOs_waves) / sizeof(LFOs_waves[0]);
...
void set_lfo_rate(uint8_t lfoNum) {
  uint8_t arrSize = sizeof(LFOs_rates) / sizeof(LFOs_rates[0]); // arrSize = 4
...
void set_LFO_gain(uint8_t lfoNum) {
...
void set_LFO_phase(uint8_t lfoNum) {
...
void set_param_trigDur() {
  uint8_t arrSize = sizeof(trig_durations) / sizeof(trig_durations[0]);
...
void set_bank_patternEdit_action() {
  uint8_t arrSize = sizeof(bank_pattern_EditAction) / sizeof(bank_pattern_EditAction[0]);
...
void set_songMode_Step() {
  uint8_t arrSize = sizeof(bank[currentBank].songPatterns) / sizeof(bank[currentBank].songPatterns[0]);
  if (enc1_dir == 1) {
...
void set_chromaMode_Arp_params_selection() {
  uint8_t arrSize = sizeof(chromaMode_Arp_params_labels) / sizeof(chromaMode_Arp_params_labels[0]);
