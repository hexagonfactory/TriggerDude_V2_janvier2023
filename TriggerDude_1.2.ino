...
#define clockInPin         PA0
#define resetInPin         PA1
#define clockOutPin        PE10
#define resetOutPin        PE11

//BUTTONS
#define SHIFT_Button  PB14
#define LIVE_Button   PD9
#define BACK_Button   PD15
#define PLAY_Button   PD13
#define REC_Button    PD11

#define BACK_led PD14
#define PLAY_led PD12
#define REC_led  PD10
#define enc_ALT_led PE1

#define encoders_ALT_Button  PC1
#define SETTINGS_Button      PC0


//ENCODERS
#define encoder1_A_Pin     PE8
#define encoder1_B_Pin     PE7
#define encoder1_Btn       PE9
#define encoder2_A_Pin     PB0
#define encoder2_B_Pin     PC5
#define encoder2_Btn       PB1


//74HC165
#define SR_165_LATCH    PB5 //PB11 sur blue pill
#define SR_165_CLOCK    PB3
#define SR_165_DATA     PB4
#define DATA_WIDTH      24
#define PADS            16

//SPISettings Settings165(16000000, MSBFIRST, SPI_MODE0);
//SPISettings Settings595(16000000, MSBFIRST, SPI_MODE0);

//74HC595
#define SR_Output_latch   PD5
#define SR_Output_clock   PD6
#define SR_Output_data    PD4
...
//HW SPI pins
//#define OLED_MOSI         PA7 // SDA (I2C) / D1 sur oled
//#define OLED_CLK          PA5 // SCL (I2C) / D0 sur oled
#define OLED_CS           PA4 // SS / NSS sur stm / CS sur oled -- Chip Select
#define OLED_DC           PC4 // Data/Command / DC sur oled         
#define OLED_RESET        PA6 // RES sur oled
U8G2_SH1106_128X64_NONAME_F_4W_HW_SPI u8g2(U8G2_R0, OLED_CS, OLED_DC, OLED_RESET);


//NEOPIXEL
#define LED_DATA PA8
Adafruit_NeoPixel strip(PADS, LED_DATA, NEO_GRB + NEO_KHZ800);

uint8_t pads_brightness = 30;
...
#define DEBOUNCE_PADS_MILLIS   15
#define SR_MICROS              1000
...
uint32_t current_pads_value;
uint32_t last_pads_value;

uint8_t current_pads_1_8_value;
uint8_t current_pads_9_16_value;
uint8_t last_pads_1_8_value;
uint8_t last_pads_9_16_value;

uint8_t  current_btns_value;
uint8_t  last_btns_value;
...
unsigned long last_pads_DebounceTime_millis[TRACKS];
...
// ALT
bool SHIFT_pushed = false;
bool enc_ALT_enabled = false;

uint8_t selectedSeqStep_byte = 0;
uint8_t selectedSeqStep_bitPos = 0;

// REMOTE BTNS
bool last_remoteBtn_SHIFT_State = 1;
bool last_remoteBtn_Back_State = 1;
bool last_remoteBtn_Play_State = 1;
bool last_remoteBtn_Rec_State = 1;
bool last_remoteBtn_LIVE_State = 1;
bool last_encoders_ALT_Btn_State = 1;
bool last_SETTINGS_Btn_State = 1;


// TRIGDUR

const uint16_t  trig_durations[4] = {1000, 2000, 5000, 10000}; // microseconds - default 2000 (2ms)
uint8_t         trig_durations_Cursor = 1;
...
uint8_t      global_Step_4PPQN = 0;
uint8_t      last_global_Step_4PPQN = global_Step_4PPQN;
uint8_t      global_Step_Counter16 = 0;
...
uint8_t  currentBankLED = 0;         // A ENLEVER QUAND STOCKAGE SUFFISANT POUR BANKS
uint8_t  currentPattern = 0;
uint8_t  currentBankDisplay = currentBank;
uint8_t  currentPatternDisplay = currentPattern;
bool  patternChange_triggered = false;
uint8_t  nextPattern = 0;

// PATTERN EDIT
const char* const bank_pattern_EditAction[] = {"RELOAD", "SAVE", "COPY", "ERASE"};
uint8_t  bank_pattern_EditAction_Cursor = 0;
bool  patternToEdit_readyToAction = 0;  // 0 si pas clické sur "ACTION > COPY / ERASE", 1 si clické
bool  bankToEdit_readyToAction = 0;
uint8_t  patternSelected = 255;
uint8_t  bankSelected = 255;
uint8_t  patternDestination = 255;
uint8_t  bankDestination = 255;
...
uint8_t clear_trigsOrPitch_cursor = 0;

// CLEAR
const char* const clear_SelectTrack_param[] = {"BOTH", "A", "B"};
uint8_t clear_SelectTrack_Cursor = 0;

//TRACK
uint8_t selected_Track = 0;

// PATTERN LENGTH
const uint8_t  pattern_LengthArray[6] = {4, 8, 16, 32, 64, 128};
bool        pattern_Length_Locked = 1;

// ROLL MODE
const uint8_t  rollRates[11] = {1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 64};
uint8_t        rollRates_cursor = 3;
unsigned long  rollRate_micros = 0;
bool           rollMode = 0;

//FILL
uint8_t fillRates[7] = {16, 12, 8, 6, 4, 3, 2}; // "1/4", "1/8", "1/16", "1/32"
uint8_t fillRate_Cursor = 0;


//SCREEN DISPLAYED
uint8_t screenConfig = 1;

bool  trigsOut_BitVal[/*DATA_WIDTH*/TRACKS]; // 16 (voir pour mettre 14 -> tracks + clockOut / resetOut)
bool  trigsIn_BitVal[/*TRACKS*/PADS];
...
// STEP SEQUENCER
uint8_t     stepSeq_page = 0;
uint8_t     stepSeq_global_Step_4PPQN = 0;
uint8_t     stepSeq_clickedStep = 255;

unsigned int decimalValue_seq = 0;
unsigned int decimalValue_seq2 = 0;


bool update_shiftRegisters[3] = {false, false, false};    // 0 = SEQ /// 1 = PADS /// 2 = CHROMA KB
...
uint8_t chromaMode_Arp_params = 0; // 0 = pas dans PARAMS, 1 = RATE, 2 = STYLE
const uint8_t arp_Rate[5] = {1, 2, 4, 8, 16};
uint8_t arp_Rate_Cursor = 2;
const char* const arp_Style[] = {"Up", "Down", "U+D"};
uint8_t arp_Style_Cursor = 0;
uint8_t arpNotes[ARP_MAX_STEPS];
uint8_t arpSteps = 0;
...
const char* const GLOBAL_Settings_titles[] = {
  "LOOP MODE",
  "FIRSTSTEP",
...
const char* const LFO_Settings_titles[] = {
  "OUTPUT",
  "WAVEFORM",
  "RATE",
  "GAIN",
...
uint8_t GLOBAL_Settings_titles_Cursor = 0;

uint8_t LFO_Settings_titles_Cursor = 0;
...
struct CVin_params {
  bool  enable = 0;
  uint8_t  dest_Cursor = 0;
  uint8_t  dest_Selected = 0;
} CVin[2];
...
struct patternStructure {
  ...
  uint8_t  glide_steps[TRACKS];                        // 12 /* nbr de steps : longueur portamento pour chaque cvOut track (4) => 0 à 16 */
  uint8_t  fill_rate[TRACKS];                          // 12
  uint8_t  loop_dir[TRACKS];                           // 12
  uint8_t  pattern_Length;                             // 1    en 4PPQN
  uint8_t  track_Length[TRACKS];                       // 12   en 4PPQN
  uint8_t  AB_State[TRACKS];                           // 12
  uint8_t  cvOut_Tracks[4];                            // 4    /* 0 à 11 = track  ---  12 = none  ---  13 = LFO[0]  ---  14 = LFO[1] */

  bool     track_isGate[TRACKS]; // 12

  uint8_t    Trigs[TRACKS_AB][MAX_STEPS_16PPQN / 8];     // 1536
  uint8_t    Pitch[TRACKS_AB][MAX_STEPS_4PPQN];          // 3072
  int8_t  transposeValue[TRACKS_AB];                  // 24  /* -2 -> +5 */

  bool      tracksRetrig;                               // 1    1 = RETRIG  ---  0 = FREE
  bool      lfo_enable[2];                              // 2
  uint8_t   lfo_dest[2];                                // 2
  uint8_t   lfo_wave[2];                                // 2
  uint8_t   lfo_rate[2];                                // 2
  uint8_t   lfo_gain[2];                                // 2
  int8_t    lfo_phase[2];                               // 2

  uint16_t  internal_BPM;                               //2
  uint8_t   clkIn_Div;                                  //1
  uint8_t   clkOut_Div;                                 //1
  bool      quantizeBeat;                               //1
  uint8_t   shuffle;                                    //1

  uint8_t    track_has_Trigs[TRACKS_AB / 8];             // 3  - track_has_Trigs[(0000.0000) (0000.0000) (0000.0000)]
  uint8_t    mute_solo_States[TRACKS_AB / 8];            // 3  - mute[(0000.0000) (0000].[0000) (0000.0000)]solo
  uint8_t    fillState[2];                               // 2  - fillState[(0000.0000) (0000./*0000*/)]
  uint8_t    firstStep;                                  // 1
...
struct bankStructure {
  bool      notEmpty_Pattern[PATTERNS];                  // 16
  uint8_t   songPatterns[16];                            // 16
  struct    patternStructure pattern[PATTERNS];          // 4800 * 12 = 57000
} bank[BANKS];                                        // total = 57628 (57000 + 12 + 16)
...
uint8_t Trigs_Undo_buffer[TRACKS_AB][MAX_STEPS_16PPQN / 8];
uint8_t Pitch_Undo_buffer[TRACKS_AB][MAX_STEPS_4PPQN];


// SONG MODE
bool songMode_Activated;
uint8_t songMode_current_Step_pot; // cursor
uint8_t songMode_current_Step_PLAYING;


uint8_t step_seq_ab[TRACKS];
...
uint8_t pitchPointer = 24;
uint8_t pitchPointer_Pot = 24;
uint8_t pitchPointer_Pad = 24;
uint8_t octaveValue = 24;


uint8_t modulated_currentPattern;
uint8_t modulated_currentBank;
uint16_t modulated_pattern_Length;
uint8_t modulated_firstStep;
uint16_t modulated_global_Step;
uint8_t modulated_lfo1Wave;
uint8_t modulated_lfo1Rate;
uint8_t modulated_lfo2Wave;
uint8_t modulated_lfo2Rate;
...
bool update_screen_POTS_DIAL_L = false;
bool update_screen_POTS_DIAL_R = false;
bool update_screen_POTS_CLK_L = false;
bool update_screen_POTS_CLK_R = false;
bool update_screen_PADS = false;
bool update_screen_SHIFT_BTN = false;
bool update_screen_ENC_ALT_BTN = false;
...
unsigned long last_sr_micros = 0;
...
void setup(void) {
  ...
  pinMode(SR_165_LATCH, OUTPUT);    //DATA LOAD PIN (PL)
  pinMode(SR_165_DATA, INPUT);    //DATA IN PIN (Q7)   --- géré par SPI.begin si SPI sur SR_165
  pinMode(SR_165_CLOCK, OUTPUT);  //CLOCK OUT PIN (CP)  --- géré par SPI.begin si SPI sur SR_165
  ...
  for (uint8_t i = 0; i < PATTERNS; ++i) {
    reset_Pattern(currentBank, i);
  }


  bank[currentBank].pattern[currentPattern].fillState[0] = B10110110;
  bank[currentBank].pattern[currentPattern].fillState[1] = B11010000;  // 10100000 0010
...
void loop(void) {
  ...
  make595Play_Trigs_from_Sequence(decimalValue_seq);
