...
uint8_t get_currentPattern_Display(uint8_t numToAdd) {
  uint8_t x = ((CVin[0].enable && (CVin[0].dest_Selected == 0)) || (CVin[1].enable && (CVin[1].dest_Selected == 0))) ? modulated_currentPattern : currentPattern;
  return x + numToAdd;
}

uint8_t get_currentBank_Display(uint8_t numToAdd) {
  uint8_t x = ((CVin[0].enable && (CVin[0].dest_Selected == 1)) || (CVin[1].enable && (CVin[1].dest_Selected == 1))) ? modulated_currentBank : currentBank;
  return x + numToAdd;
}

uint8_t get_track_Step_Display_4PPQN(uint8_t track, bool stepToAdd) {
  uint8_t x = ((CVin[0].enable && (CVin[0].dest_Selected == 2)) || (CVin[1].enable && (CVin[1].dest_Selected == 2))) ? modulated_global_Step : track_Step_16PPQN[track] / 4;
  return x + stepToAdd;
}

uint8_t get_current_Step_Display_4PPQN(bool stepToAdd) {
  uint8_t x = ((CVin[0].enable && (CVin[0].dest_Selected == 2)) || (CVin[1].enable && (CVin[1].dest_Selected == 2))) ? modulated_global_Step : global_Step_4PPQN;
  return x + stepToAdd;
}

uint16_t get_pattern_Length(uint8_t PPQN) {
...
uint16_t get_track_Length(uint8_t track, uint8_t PPQN) {
...
uint16_t get_firstStep(uint8_t PPQN) {
  uint8_t multip = (PPQN == 16) ? 4 : 1;
...
uint8_t get_LFO_wave_Display(bool lfoNum) {
  uint8_t x = ((CVin[0].enable && CVin[0].dest_Selected == 5) || (CVin[1].enable && CVin[1].dest_Selected == 5)) ? ((lfoNum == 0) ? modulated_lfo1Wave : modulated_lfo2Wave) : bank[currentBank].pattern[currentPattern].lfo_wave[lfoNum];
  return x;
}


uint8_t get_LFO_rate_Display(bool lfoNum) {
  uint8_t x = ((CVin[0].enable && CVin[0].dest_Selected == 6) || (CVin[1].enable && CVin[1].dest_Selected == 6)) ? ((lfoNum == 0) ? modulated_lfo1Rate : modulated_lfo2Rate) : bank[currentBank].pattern[currentPattern].lfo_rate[lfoNum];
  return x;
}


uint8_t get_LFO2_rate_Display() {
  uint8_t x = ((CVin[0].enable && CVin[0].dest_Selected == 8) || (CVin[1].enable && CVin[1].dest_Selected == 8)) ? modulated_lfo2Rate : bank[currentBank].pattern[currentPattern].lfo_rate[1];
  return x;
}


uint16_t get_track_step_TRIGS(uint8_t track, bool step_advance) {
...
uint8_t get_track_step_PITCH(uint8_t track, bool step_advance) {
  ...
  uint8_t loop_LengthDisplay = get_track_Length(track, 4);
  /*uint8_t trackStepVal = (shiftedStep < 0) ? (shiftedStep + pattern_LengthDisplay) : (shiftedStep > pattern_LengthDisplay - 1) ? (shiftedStep - pattern_LengthDisplay) : shiftedStep;
    return trackStepVal;*/
...
void read_Sequence_Trigs() {
  for (uint8_t track = 0; track < TRACKS; ++track) {
    ...
    if ( (screenConfig == 1 || screenConfig == 6 || screenConfig == 8 || screenConfig == 33) && (read_Seq_dont_play_this[track] || trigsIn_BitVal[track]) ) {
      can_Play = false;
  ...
  decimalValue_seq = 0;
  //int j;
  for (int8_t i = TRACKS - 1, j = 0; i > -1; i--) {
    decimalValue_seq += trigsOut_BitVal[j++] << i;
  }

  if (screenConfig == 1)
    decimalValue_seq |= decimalValue_from_pads();

  else if (screenConfig == 33)
    decimalValue_seq |= decimalValue_from_chromaMode();
...
void read_Sequence_Trigs_next() {
  ...
  if (screenConfig == 1)
    decimalValue_seq2 |= decimalValue_from_pads();

  else if (screenConfig == 33)
    decimalValue_seq2 |= decimalValue_from_chromaMode();

  //decimalValue_seq2 *= 16;
}
...
void startUpSequence() {
  ...
  for (uint8_t i = 0; i < PADS; ++i) {
    if (i != 0)
      strip.setPixelColor(i, (45 + ((i + 10) * 3)), 101 + (i * 7), (168 - ((i + 10) * 2)));
    else
      strip.setPixelColor(i, 45, 101, 168);
    strip.show();
    delay(80);
...
void modulate_currentPattern(uint8_t cvInNum) {
...
void modulate_currentBank(uint8_t cvInNum) {
  modulated_currentBank = map(analogRead(cvInNum), 0, 1023, BANKS - 1, -(BANKS - 1));
  if (modulated_currentBank > 11) modulated_currentBank = 11;
}


void modulate_pattern_Length(uint8_t cvInNum, uint16_t lastPatLength) {
...
void modulate_firstStep(uint8_t cvInNum) {
...
void modulate_current_Step(uint8_t cvInNum) {
...
bool checkIf_trigs(uint8_t old_patLength, uint8_t new_patLength) { // checkIf_trigs(last_pattern_Length, bank[currentBank].pattern[currentPattern].pattern_Length);
  ...
  for (uint8_t track = 0; track < TRACKS_AB; ++track) {
...
void checkIf_patternEmpty(uint8_t ze_bank, uint8_t ze_pattern) {
  for (uint8_t track = 0; track < (TRACKS_AB); ++track) {
...
void checkIf_glide_running(bool glideIsSetToActive) {
  for (uint8_t dac = 0; dac < 4; ++dac) {
    uint8_t cvTrack = bank[currentBank].pattern[currentPattern].cvOut_Tracks[dac];
...
void reset_Pattern(uint8_t ze_bank, uint8_t ze_pattern) {
  for (uint8_t track = 0; track < TRACKS_AB; ++track) {
...
void resetBank(uint8_t ze_bank) {
  for (uint8_t pat = 0; pat < PATTERNS; ++pat) {
    reset_Pattern(ze_bank, pat);
  }

  for (uint8_t songStep = 0; songStep < 16; ++songStep) {
...
uint32_t get_LFO_Rate_micros(uint8_t lfoNum) {
...
void buffer_Sequences() {
  ...
  for (uint8_t track = 0; track < TRACKS_AB; ++track) {
    for (uint8_t steps = 0; steps < MAX_STEPS_4PPQN; ++steps) {
...
void clearSequence(uint8_t zebank, uint8_t zepattern, uint8_t track, uint16_t steps, bool delete_trigs, bool delete_pitch) {
  if (delete_trigs == 1) {
    Trigs_Undo_enable = 1;
...
void assign_CVout_track(uint8_t padID) {
  uint8_t cvTrack = bank[currentBank].pattern[currentPattern].cvOut_Tracks[padID];
...
bool LFO_enable_State(uint8_t lfoNum) {
  bool enable_State;
  uint8_t lfoTrack = (lfoNum == 0) ? 13 : 14;
  for (uint8_t cvOut = 0; cvOut < 4; ++cvOut) {
