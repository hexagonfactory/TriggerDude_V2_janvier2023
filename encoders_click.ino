...
void detect_lockpattern_LengthOnOff() {
  if (pattern_Length_Locked == 1) {
    ...
    if (SHIFT_pushed) {     // vérifier pourquoi ALTENABLE ???
      for (uint8_t i = 0; i < sizeof(pattern_LengthArray) / sizeof(pattern_LengthArray[0]); ++i) {
...
void detect_Track_AB_change(uint8_t curTrack) {
...
void detect_resetABStates() {
  uint8_t x = (bank[currentBank].pattern[currentPattern].allAorB) ? 12 : 0;
  for (uint8_t track = 0; track < TRACKS; ++track) {
...
void detect_clearPitch(uint8_t curTrack) {
  for (uint8_t i = 0; i < MAX_STEPS_4PPQN; ++i) {
...
/*void detect_reload_pattern(uint8_t zeBank, uint8_t zePattern) {
...
void detect_lfo_onOff(uint8_t lfoNum) {
...
void detect_reset_LFO_PHASE(uint8_t lfoNum) {
...
void detect_cvOutReassign(uint8_t cvOutput, uint8_t track) { // fonction a modifier (enc1_clk = false; etc)
