void set_UI_Config() {
  ...
  switch (screenConfig) {
    ...
    case 7: // SETTINGS SCREEN
      ...
      GLOBAL_Settings_encoders_events();

      drawPage_GLOBAL_Settings();
      break;
    ...
    case 17: // LFO 1 SETTINGS SCREEN
      makeStripDisplay_Nothing();

      //LFO_Settings_encoders_events(0);

      drawPage_LFO_Settings(0);
      break;


    case 18: // LFO 2 SETTINGS SCREEN
      makeStripDisplay_Nothing();

      LFO_Settings_encoders_events(1);

      drawPage_LFO_Settings(1);
      break;
...
void GLOBAL_Settings_encoders_events() {
  if (fired_1)
    set_GLOBAL_Settings_selection();
...
void LFO_Settings_encoders_events(bool lfoNum) {
  if (fired_1)
    set_LFO_Settings_selection(/*lfoNum*/);
  ...
  switch (LFO_Settings_titles_Cursor) {
    ...
    case 4: // set LFO N PHASE
      if (fired_2)
        set_LFO_phase(lfoNum);

      if (bank[currentBank].pattern[currentPattern].lfo_phase[lfoNum] != 0) {
        if (enc2_clk)
          detect_reset_LFO_PHASE(lfoNum);
      }
      break;
